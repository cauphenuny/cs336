...
def tokenize(tokenizer_path, file, num_chunks=0, num_threads=0, dtype=np.int16, vocab_size=10000):
    specials = ["<|endoftext|>"]
    tokenizer = Tokenizer.from_files(f"{tokenizer_path}-vocab.json", f"{tokenizer_path}-merges.json", specials)
    logger.info("reading")
    ...
    with open(file, "rb") as f:
        ...
        for i, (start, end) in enumerate(zip(boundaries[:-1], boundaries[1:])):
            ...
            if np.max(token) >= vocab_size:
                logger.error(f"tokenizer produced token {np.max(token)} >= vocab size {vocab_size}")
    ...
    if np.max(final) >= vocab_size:
        logger.error(f"final array has token {np.max(final)} >= vocab size {vocab_size}")
